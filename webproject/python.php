...
<body style="background-image: url('img/backgrnd1.jpg'); background-size: 100%;">
    ...
    <?php
      ...
      $query = "SELECT * FROM content_info WHERE course_name = 'python' ";
    ...
    <div class="main-menu" style="background-color:powderblue;">
      <ul>
        <h5><a style="text-decoration:none" href="python.php">Home</a></h5>
    ...
    <?php
      ...
      while ($row = mysqli_fetch_assoc($fetchData))
      {
        $content_name = $row['content_name'];   
        ?>
        <li class="menu-item"> <a href="python.php?topic=<?=$content_name; ?>"><?=$content_name; ?></a></li>
        <?php  
      }
    ...
    <?php
      $blog = "<h3>Welcome to python!!</h3>";
      if(isset($_GET['topic']))
      {
        $topic = $_GET['topic'];
        $query = "SELECT * FROM content_info WHERE course_name = 'python' and content_name= '$topic'";
        $fetchData = mysqli_query($connect, $query);
