<?php session_start();

if(!isset($_SESSION['username'])){
    echo "you are logged out";
    header('location:loginAndregistration.php');
}
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="data:image/png;base64," rel="icon" type="image/x-icon">
    <title>Home</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">

    <?php
        include "navbar.php";
    ?>
    <style>
        table, td, th {  
        border: 0.5px solid #ddd;
        text-align: left;
        }

        table {
        border-collapse: collapse;
        width: 50%;
        }

        th, td {
        padding: 15px;
        }
        table.center {
        margin-left: auto; 
        margin-right: auto;
        }
    </style>
</head>

<body style="background-image: url('img/backgrnd1.jpg'); background-size: 100%;">
    <br>

    <h2>&nbsp; Welcome <?= $_SESSION['username']; ?> to Compurial.</h2>
    <br>
    

    <br>

    
     <div class="container-fluid" width="50%">
        <div class="row">
            <div class="col-lg-4 col-md-4 col-12">
                <div class="card">
                    <img src="img/python.png" class="card-img-top easyimg" alt="Card Image">
                    <div class="card-body">
                        <a href="python.php" class="btn btn-primary"> Python</a>
                        <p class="card-text">Python is an interpreted high-level general-purpose programming language. Its design philosophy emphasizes code readability with its use of significant indentation. Its language constructs as well as its object-oriented approach aim to help programmers write clear, logical code for small and large-scale projects.</p>
                    </div>
                </div>
            </div>

            <div class="col-lg-4 col-md-4 col-12">
                <div class="card">
                    <img src="img/ML.png" class="card-img-top easyimg" alt="Card Image">
                    <div class="card-body">
                        <a href="ml.php" class="btn btn-primary">Machine Learning</a>
                        <p class="card-text">Machine learning is a method of data analysis that automates analytical model building. It is a branch of artificial intelligence based on the idea that systems can learn from data, identify patterns and make decisions with minimal human intervention.</p>
                    </div>
                </div>
            </div>

            <div class="col-lg-4 col-md-4 col-12">
                <div class="card">
                    <img src="img/CP.png" class="card-img-top easyimg" alt="Card Image">
                    <div class="card-body">
                        <a href="cp.php" class="btn btn-primary">Competitive Programming</a>
                        <p class="card-text">Competitive Programming is a mental sport which enables you to code a given problem under provided constraints. The purpose of this article is to guide every individual possessing a desire to excel in this sport. This article provides a detailed syllabus for Competitive Programming boost the preparation of the readers.</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <br>
    <br>
    <br>
    <br>

    <div style="text-align: center;">
    <Label><b>Last Viewed</b></Label>
    <table class="center">
        <tr>
            <td>
                Your last opened page in Python is:   
            </td>
            <td>
                <?php
                    if(isset($_COOKIE['python'])){
                        $cokie = $_COOKIE['python'];
                        ?>
                        <a href="python.php?topic=<?=$cokie ; ?>"><?=$cokie; ?></a>
                        <?php
                    }
                ?>
            </td>
        </tr>
        <tr>
            <td>
                Your last opened page in Machine Learning is:   
            </td>
            <td>
                <?php
                    if(isset($_COOKIE['ml'])){
                        $cokie = $_COOKIE['ml'];
                        ?>
                        <a href="ml.php?topic=<?=$cokie ; ?>"><?=$cokie; ?></a>
                        <?php
                    }
                ?>
            </td>
            
        </tr>
        <tr>
             <td>
                Your last opened page in Competitive Programming is:   
            </td>
            <td>
                <?php
                    if(isset($_COOKIE['cp'])){
                        $cokie = $_COOKIE['cp'];
                        ?>
                        <a href="cp.php?topic=<?=$cokie ; ?>"><?=$cokie; ?></a>
                        <?php
                    }
                ?>
            </td>
        </tr>
    </table>
    </div>

    <br>
    <br>
    <br>
    <br>


    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script>
</body>
</html>